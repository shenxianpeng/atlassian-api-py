...
class TestBitbucket(unittest.TestCase):
    ...
    def test_get_project_repo_name(self):
        """Test to get project repository name"""
    ...
    def test_get_repo_info(self):
        """Test to get repository information"""
    ...
    def test_get_repo_branch(self):
        """Test to get a repository branch"""
        ...
        for branch in branches:
            ...
            if branch.displayId == "hotfix/12.1.1.HF1":
                self.assertEqual(
                    branch.latestCommit, "da7696ceacaff863d40a3215eea08e661d5b1c5a"
                )
        exist_branches = [
            "hotfix/12.1.1.HF1",
            "hotfix/12.1.1.HF2",
    ...
    def test_create_delete_branch(self):
        """Test to create and delete branch"""
        # Create branch failed because of When performing a ref operation, the author must have an e-mail address.
        CREATE_BRANCH = "feature/create_branch_with_rest_api"
        self.git.create_branch("MVAS", "uvuddb", CREATE_BRANCH, "da7696ceacaff")
        branch_names = self.git.get_repo_branch("MVAS", "uvuddb")
        self.assertNotIn(CREATE_BRANCH, branch_names)
        self.git.delete_branch("MVAS", "uvuddb", CREATE_BRANCH, "da7696ceacaff")
        self.assertNotIn(CREATE_BRANCH, branch_names)

    def test_get_merged_branch(self):
        """Test to get merged branches"""
    ...
    def test_get_branch_commits(self):
        """Test to get a specific branch commits"""
        commits = self.git.get_branch_commits(
            "MVAS", "uvuddb", "hotfix/12.1.1.HF1", limit=1
        )
    ...
    def test_get_branch_committer_info(self):
        """Test to get a branch committer information"""
        all_committer = self.git.get_branch_committer_info(
            "MVAS", "uvuddb", "hotfix/12.1.1.HF1", limit=0
        )
        for committer in all_committer:
            self.assertEqual("Xianpeng Shen", committer.displayName)

    def test_get_pull_request(self):
        """Test to get a pull requets information"""
    ...
    def test_get_pull_request_source_branch_name(self):
        """Test to get a pull request's source branch name"""
        pr_source_branch_name = self.git.get_pull_request_source_branch_name(
            "MVAS", "uvuddb", "670"
        )
        self.assertEqual(pr_source_branch_name, "hotfix/12.1.1.HF6")

    def test_get_pull_request_destination_branch_name(self):
        """Test to get a pull request destination branch name"""
        pr_destination_branch_name = self.git.get_pull_request_destination_branch_name(
            "MVAS", "uvuddb", "670"
        )
        self.assertEqual(pr_destination_branch_name, "uv1212-dev")

    def test_get_pull_request_relate_jira_key(self):
        """Test to get a pull request relate jira key"""
        relate_jira_key = self.git.get_pull_request_relate_jira_key(
            "MVAS", "uvuddb", "671"
        )
        self.assertEqual(relate_jira_key, "UNV-30005")

    def test_get_pull_request_id(self):
        """Test to get a pull request ID"""
        pr_ids = self.git.get_pull_request_id(
            "MVAS", "uvuddb", pr_state="MERGED", limit=0
        )
        for pr_id in pr_ids:
            self.assertGreaterEqual(pr_id, 671)

    def test_get_pull_request_diff(self):
        """Test to get a pull request diff information"""
    ...
    def test_get_pull_request_activities(self):
        """Test to get a pull request activities information"""
    ...
    def test_get_pull_request_merge(self):
        """Test to get a pull request merge information"""
    ...
    def test_get_pull_request_commits(self):
        """Test to get a pull request commits"""
    ...
    def test_update_pull_request_comment(self):
        """Test to update pull request comment"""
        self.git.update_pull_request_comment(
            "MVAS", "uvuddb", 585, "test", "update to test"
        )

    def test_delete_pull_request_comment(self):
        """Test to delete pull request comment"""
        self.git.delete_pull_request_comment("MVAS", "uvuddb", 585, "update to test")

    def test_get_file_change_history(self):
        """Test to get file change history of a branch"""
    ...
    def test_get_file_content(self):
        """Test to get file content of a branch"""
        file_content = self.git.get_file_content(
            "MVAS", "uvuddb", "release/12.1.1.HF6.PE", ".gitignore"
        )
        self.assertIn("# windows ignorance files", file_content)

    def test_get_build_status(self):
        """Test to get bulid status"""
